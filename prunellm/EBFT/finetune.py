    ...
    with torch.no_grad():
        ...
        for inputs, outs in tensordata_loader:
            with torch.amp.autocast("cuda"):
                outputs = layer(inputs, attention_mask=attention_mask, position_ids=position_ids)[0]
...
def train(layer, inps, outs, dataloader, args, device, attention_mask, position_ids, layer_index=None):
    ...
    plt.ylabel('loss')
    plt.savefig(f'plots/loss_plot_{layer_index}.png')
    plt.clf()
    plt.plot(list(range(0,len(lrs))), lrs)
    plt.xlabel('training steps')
    plt.ylabel('lr')
    plt.savefig(f'plots/lr_plot_{layer_index}.png')
    plt.clf()
