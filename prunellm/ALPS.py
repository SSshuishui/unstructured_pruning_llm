...
class ALPSGPT:
    ...
    def ALPS_admm(self, sparsity, prunen=0, prunem=0, rho=0.1, max_iter = 300, update_iter = 3, switch_iter = 30):
        ...
        if prunen == 0:
            ...
            D = B.clone().t().reshape((new_dim, prunem))
            _, loss_idx = torch.topk(-D**2,prunem - prunen, dim = 1)
            D = D.scatter(src=torch.zeros((new_dim,prunem-prunen)).to('cuda:0'),dim=1,index=loss_idx)   
            D_suppp = (D == 0).to(torch.float)
        ...
        for i_admm in range(max_iter):
            ...
            if fix_supp:
                ...
                D = ((V + rho * B) / rho).t().reshape((new_dim, prunem))
                _, loss_idx = torch.topk(-D**2,prunem - prunen, dim = 1)
                D = D.scatter(src=torch.zeros((new_dim,prunem-prunen)).to('cuda:0'),dim=1,index=loss_idx) 
            ...
            if (i_admm+1) % update_iter == 0:
                ...
                if prunen == 0:
                    ...
                    Btest = B.t().reshape((new_dim, prunem))
                    _, loss_idx = torch.topk(-Btest**2,prunem - prunen, dim = 1)
                    Btest = Btest.scatter(src=torch.zeros((new_dim,prunem-prunen)).to('cuda:0'),dim=1,index=loss_idx)  
        ...
        if prunen == 0:
            ...
            B = B.t().reshape((new_dim, prunem))
            _, loss_idx = torch.topk(-B**2,prunem - prunen, dim = 1)
            B = B.scatter(src=torch.zeros((new_dim,prunem-prunen)).to('cuda:0'),dim=1,index=loss_idx)  
