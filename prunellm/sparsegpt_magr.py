...
class MagRGPT:
    ...
    def fasterprune(
        self, sparsity, prunen=0, prunem=0, blocksize=128, groupsize=-1, percdamp=.01, magr=False, CD_iter=1,
    ):
        ...
        print('comq', CD_iter)
        W_CD = W.reshape(self.layer.weight.shape).clone()
        ...
        for _ in range(CD_iter):
            delta_W = W_CD.clone().t()     
            P_hat = torch.matmul(norm_Sigma, delta_W).t()
                
            for j in range(self.columns):
                u = P[:, j] - P_hat[:, j]
                if j > 0:
                    u += torch.matmul(norm_Sigma[j, :j], delta_W[:j, :])

                if groupsize != -1:
                    if j % groupsize == 0:
                        self.quantizer.find_params(W_CD[:, j:(j + groupsize)], weight=True)
        ...
        if isinstance(self.layer, transformers.Conv1D):
            W = W.t()
        self.layer.weight.data = W_CD.reshape(self.layer.weight.shape).to(self.layer.weight.data.dtype)
        if DEBUG:
            print(torch.sum((self.layer(self.inp1) - self.out1) ** 2))
